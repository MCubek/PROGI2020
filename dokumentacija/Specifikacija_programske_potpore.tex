		...
		\item pristupiti pregledu rang ljestvice svih igrača
			...
			\begin{packed_enum}
				...
				\begin{packed_enum}
					...
					\item izmijeniti prijavu te zatim lokaciju prihvatiti
					\item zatražiti potvrdu s terena
				\end{packed_enum}
			\end{packed_enum}
...
\item  \underbar{OSRM (sudionik) :}
		\begin{packed_enum}
			\item omogućava pronalazak najkraćeg puta obilaska svih lokacija za koje je zatražena potvrda s terena
		\end{packed_enum}
			\end{packed_enum}
		...
		\end{figure}
			\subsection{Sekvencijski dijagrami}
				...
				\subsubsection{Obrazac uporabe UC8 - Borba}
				\textnormal{Igrač šalje zahtjev za borbom. Web aplikacija potom dohvaća suparnika. U bazi podataka se provjerava ima li igrač barem 5 karata za borbu dostupnih. Ukoliko igrač nema dovoljno karata za borbu ne može pokrenuti "Traži borbu", web aplikacija izbacuje poruku. Iz baze podataka dohvaća se vlastita kolekcija karata igrača te se prikazuje ekran za odabir 5 karata za borbu. Nakon odabira igrač potvrđuje izabranih (označenih) pet karata i borba započinje bacanjem novčića kako bi se odlučilo kreće li igrač prvi ili ne koje vrši web aplikacija. Nakon odluke, igra se 5 rundi. Igrač koji započinje potez u rundi bira kartu i jedan od tri atributa za tu rundu dok igrač koji kreće drugi izabire samo kartu, a odabir atributa je isti kao od igrača koji je započeo rundu. Nakon što su karta i njen atribut odabrani, gleda se čiji atribut je veći te taj pobjednik pobjeđuje rundu. Poslije 5 odigranih rundi proglašuje se pobjednik i u bazu podataka se spremaju podaci te borbe.}\\
				\begin{figure}[H]
					\centering
			...
			\subsubsection{Obrazac uporabe UC12.1}
		...
		\section{Ostali zahtjevi}
			...
			\textbf{\textit{dio 1. revizije}}\\
			 ...
			 \begin{packed_item}
			 	\item  \textbf{Sustav treba omogućiti rad više korisnika u stvarnom vremenu}
			 	\item  \textbf{Korisničko sučelje i sustav moraju podržavati hrvatsku abecedu (dijakritičke znakove) pri unosu i prikazu tekstualnog sadržaja}
			 	\item  \textbf{Sustav mora imati kroz igranje pasivno poticati igrača na kretanje i sakupljanje drugih lokacija (karata)}
			 	\item  \textbf{Dijelovi sustava gdje se koristi baza podataka trebaju biti fluidni i brzi (manje od 1 sekunde)}
			 	\item  \textbf{Sustav treba biti implementiran koristeći Angular za frontend i Spring Boot za backend, dok se za bazu podataka koristi Posgresql}
			 	\item  \textbf{Svi vizuali aplikacije moraju biti jednostavni i ugodni oku}
			 	\item  \textbf{Sustav treba biti intuitivan i jednostavan}
			 	\item  \textbf{Pristup sustavu mora biti omogućen iz javne mreže pomoću HTTPS}
			 	\item  \textbf{Veza s bazom podataka mora biti kvalitetno zaštićene, brza i otporna na vanjske greške}
			 	\item  \textbf{Sustav kao valutu za isplatu kartografa koristi EUR}
